...
Linux 服务持续采集并写入 `ALERT_DATA_DIR/market.sqlite`：海力士报价每 10 秒、原油报价默认每 30 秒、海力士小时历史与原油 15 分钟历史每 60 秒、原油旧日线与两类已结算资金费每 5 分钟。首次导入仓库中的真实历史，之后从数据库接续增量更新并回补缺口；行情快照和逐条观测在同一事务内保存。页面只读取已保存的数据，打开、刷新和切换页面都不会调用交易所。没有任何页面打开时也会继续采集，进程重启后立即可读上次记录。
...
- `GET /api/monitors/oil/candles/15m`：原油同一 UTC 时段已收盘的 15 分钟 K 线。Linux 只读独立数据集 `oil/candles/15m`，原日线接口与数据库记录保留。
...
- 原油为 `xyz:BRENTOIL − xyz:CL`。WTI 界面名为 WTIOIL。当前价差用 `markPx`，资金费现金流用 `oraclePx`，费率是每小时小数率。主图与顶部迷你图采用两腿同一 UTC 时段的 **15 分钟 K 线收盘价差**，默认近 1 周，可选 1 天、1 周、1 月及全部已存历史；图表显示北京时间与具体时分，未收盘 K 线不纳入，缺口断线。单腿最高、最低价并非同时出现，因此不将两腿高低价直接相减构造价差蜡烛。
- [Hyperliquid candleSnapshot](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/info-endpoint#candle-snapshot) 只提供最近约 5,000 根 K 线，15 分钟约覆盖 52 天。初始真实归档位于 `public/oil/data/hyperliquid-15m.json`；后台从数据库增量采集，重取最近 24 小时并回补可恢复的缺口，超过交易所回溯范围的已存历史仍保留。不会从日 K 插值生成更早的 15 分钟数据。
- 原油历史面板同时显示做多、做空价差的区间累计资金费及累计年化，并可切换累计年化曲线和日均小时费率。按上方精确时间区间纳入全部共同整点结算（即使某时段缺少价格 K 线），采用等预言机美元名义、两腿总敞口作分母；年化为净小时费率之和除以有效小时数再乘 `8760`，不复利。切换区间重新累计，曲线按 UTC 日汇总并标在区间内当日最后一次结算的位置；日均值按实际小时平均，不补零，并显示覆盖及缺失小时数。15 分钟明细仅在整点行显示该时点资金费，避免重复计入。
...
- 保留原仓库的真实快照和边界：原油旧日线及资金费历史范围目前为 2026 年，15 分钟价格历史持续积累；海力士自上市后的首个完整小时开始。缺失数据保留断点。
