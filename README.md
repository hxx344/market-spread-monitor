...
“合约价差”模块在常驻 Node 后台自动发现各平台正在交易的线性永续合约，使用 WebSocket 获取报价；REST 用于合约目录、计价单位与资金费周期等元数据。市场目录每 30 分钟刷新，连接失败自动退避重连。仅接入公共行情，无需交易账户或钱包。

浏览器通过同源、已有登录保护的 SSE 接收每秒合并增量，首次连接及每 30 秒校准完整快照，支持流式 gzip；代理不支持 SSE 时降级为每 5 秒读取后台快照，不重复访问交易所。切换面板或隐藏页面暂停浏览器更新，后台采集继续运行。开发网页预览无常驻采集时明确显示未连接，请使用下方 Windows 完整后台命令，或 Linux 一键部署命令启动。
...
最新合约快照每 5 秒增量保存于 `ALERT_DATA_DIR/perpetual/market.sqlite`，仅保留每个合约的最新记录；重启可显示原时间的旧报价，连接恢复后更新。服务获取成功的新目录会移除已下线市场，数据库与配置位于版本目录之外，现有一键升级流程自动保留，无需新增依赖或手动迁移。
