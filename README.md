...
| 合约价差 | Binance / Bybit / OKX / Bitget / Gate 与 Lighter / RH-Lighter / Hyperliquid / Aster / Entropy 的公开永续行情、组合毛 / 净价差排名、按金额盘口检查、持有期情景、组合自选、持续条件提醒与报价健康 |
...
价差是盘口顶层的毛价差，不包含手续费、滑点、市场深度及资金费变化。同一币种的不同做多/做空平台与合约组合各占一行，可按毛价差或扣除 taker 往返费及滑点预算后的净价差排序，最低阈值跟随所选口径；缺失手续费不参加净排序。每页 30 个组合，组合可独立收藏或屏蔽，旧币种自选保留。展开后用“成交与持有 / 质量依据 / 各平台报价”切换详情；不执行交易。原油、海力士原有图表与飞书规则照常保留。
...
“聚合质量”结合代币市值、FDV、多空账户占比、同一组合近 1 小时价差稳定度及近 24 小时预估资金费稳定度，展开可查看分项、数据覆盖、来源和时间。缺失数据和启动初期样本不足不补零，不显示为高质量。市值低频批量读取，多空比例仅为当前观察组合排队采集；原有行情订阅不增加。新增质量历史最多保存 24 小时 / 1440 个压缩分钟桶，自动淘汰旧数据；前 24 小时增长到滚动窗口后复用空间，无需定时清理。手续费按两腿平台及合约的公开普通账户 taker 费率计算，开平仓各计一次；VIP 或优惠费率可逐平台覆盖，滑点默认 0.10% 可修改。展开可见单腿费率、来源与往返合计；未知不当零费用。费用复用现有目录刷新，不增加查询轮询或历史表。权重、公式、接口范围及资源验证见 [聚合质量说明](docs/PERPETUAL_QUALITY.md)。
