...
CrossEx 的 `opportunities-v2` 继续返回兼容的完整 JSON，支持标准 HTTP gzip。报价、目录或汇率版本变化时重新计算；源版本相同的投影最多复用 10 秒。信号的 `observedAt/expiresAt` 始终来自原报价与汇率时间，响应时间不会延长可用期限；重启会丢弃投影缓存并重新生成完整结果。解压后的响应大小仍由 CrossEx 限制。
