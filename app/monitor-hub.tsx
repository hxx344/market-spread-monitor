...
export default function MonitorHub({ initial = null }: { initial?: InitialMarketData | null }) {
  ...
  return <div className="monitor-hub">
    ...
    <Tabs value={active} onValueChange={value => selectMonitor(String(value))} className={`hub-tabs ${active === "perpetual" ? "hub-perpetual-active" : ""}`}>
      ...
      {monitors.filter(monitor => monitor.id === "oil" || monitor.id === "hynix").map(monitor => { const id = monitor.id as "oil" | "hynix"; return <div key={id} hidden={active !== id} className="hub-exchange-comparison"><ExchangeComparison monitorId={id} primary={summaries[id]?.comparison} initial={initial?.[id].exchanges} renderedAt={initial?.renderedAt}/></div>; })}
