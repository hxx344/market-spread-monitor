...
export default function MonitorHub({ initial = null }: { initial?: InitialMarketData | null }) {
  ...
  return <div className="monitor-hub">
    ...
    <Tabs value={active} onValueChange={value => setActive(String(value))} className="hub-tabs">
      <TabsList className="hub-tab-list" aria-label="选择监控市场">{monitors.map(monitor => <TabsTrigger key={monitor.id} value={monitor.id} className="hub-tab" aria-label={monitor.title}><span className="hub-card-heading"><i style={{background:monitor.accent}}/><span>{monitor.title}<small>{monitor.subtitle} · {monitor.id === 'oil' ? 'Binance' : 'Hyperliquid'}</small></span><em>{monitor.category}</em></span>{summaries[monitor.id] && <CardSummary summary={summaries[monitor.id]} intervalMs={monitor.quoteIntervalMs} renderedAt={initial?.renderedAt} />}</TabsTrigger>)}</TabsList>
      {monitors.map(monitor => { const id = monitor.id as "oil" | "hynix"; return <div key={id} hidden={active !== id} className="hub-exchange-comparison"><ExchangeComparison monitorId={id} primary={summaries[id]?.comparison} initial={initial?.[id].exchanges} renderedAt={initial?.renderedAt}/></div>; })}
