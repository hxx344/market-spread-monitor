  ...
  return <section className="perp-trend" aria-label="该组合近一小时价差走势">
    ...
    {crossCurrency ? <p className="perp-detail-muted">跨计价币暂不绘制历史，避免混用换汇前后的口径。</p> : chart ? <>
      <svg viewBox="0 0 740 165" role="img" aria-label={`分钟采样毛价差，最低 ${pct(Math.min(...chart.valid.map(p => p[1])))}，最高 ${pct(Math.max(...chart.valid.map(p => p[1])))}，缺失分钟断线`}>
        {[chart.min, (chart.min + chart.max) / 2, chart.max].map(value => <g key={value}><line x1="54" x2="712" y1={chart.y(value)} y2={chart.y(value)} stroke="#2b404f"/><text x="47" y={chart.y(value) + 3} textAnchor="end" fill="#97acbc" fontSize="10">{value.toFixed(2)}%</text></g>)}
        <path d={chart.path} fill="none" stroke={stale ? "#97acbc" : "#8dbbff"} strokeWidth="2"/>
        {chart.valid.map(([at, value]) => <circle key={at} cx={chart.x(at)} cy={chart.y(value)} r="1.8" fill="#8dbbff"><title>{time.format(at)} · {pct(value)}</title></circle>)}
        <text x="54" y="154" fill="#97acbc" fontSize="10">{time.format(chart.start)}</text><text x="712" y="154" textAnchor="end" fill="#97acbc" fontSize="10">{time.format(chart.end)} 北京时间</text>
