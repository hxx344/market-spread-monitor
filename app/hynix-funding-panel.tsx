  ...
  return <section className="chart-panel hynix-funding-panel" aria-label="海力士历史多空资金费">
    ...
    <div className="chart-container funding-curve" aria-label={`${selectedMode.label}曲线`}>
      {!summary.count ? <div className="empty-chart" role="status"><p>{loading ? "正在载入历史结算费率…" : "所选区间没有共同结算数据"}</p><span>{error || "仅计算双方都有费率的结算小时"}</span></div> : <ResponsiveContainer width="100%" height="100%" minWidth={0} initialDimension={{ width: 800, height: 280 }}><LineChart data={summary.chart} margin={{ top: 15, right: 12, left: 0, bottom: 8 }} accessibilityLayer>
        <CartesianGrid stroke="#27313b" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="time" type="number" domain={[summary.firstTime! - (summary.expectedHours === 1 ? FUNDING_HOUR / 2 : 0), summary.lastTime! + (summary.expectedHours === 1 ? FUNDING_HOUR / 2 : 0)]} tickFormatter={shortDate} minTickGap={65} stroke="#81909f" axisLine={false} tickLine={false} tick={{ fontSize: 12 }}/>
        <YAxis orientation="right" width={76} tickFormatter={value => `${Number((value * 100).toPrecision(3))}%`} domain={[(min: number) => Math.min(-0.00001, min), (max: number) => Math.max(0.00001, max)]} stroke="#81909f" axisLine={false} tickLine={false} tick={{ fontSize: 12 }}/>
        <ReferenceLine y={0} stroke="#728494" strokeDasharray="4 4"/><Tooltip content={<FundingTooltip/>} cursor={{ stroke: "#667b8d", strokeDasharray: "3 3" }} wrapperStyle={{ maxWidth: "100%" }}/>
        <Line dataKey={selectedMode.short} name="做空价差" type="linear" stroke="#65dfb8" strokeWidth={1.8} dot={summary.missingHours ? { r: 1.5 } : summary.count === 1} activeDot={{ r: 4 }} connectNulls={false} isAnimationActive={false}/><Line dataKey={selectedMode.long} name="做多价差" type="linear" stroke="#85b6ff" strokeDasharray="5 3" strokeWidth={1.8} dot={summary.missingHours ? { r: 1.5 } : summary.count === 1} activeDot={{ r: 4 }} connectNulls={false} isAnimationActive={false}/>
