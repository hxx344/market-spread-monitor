  ...
  try {
    ...
    return {
      ...
      write(id, action, input, { seed = false } = {}) {
        ...
        let value, sourceMs;
        try {
          value = validators[key](action === 'quote' ? input : { ...input, status: seed ? 'snapshot' : 'live' });
        ...
        const rows = action === 'quote' ? [{ time: sourceMs, ...value }] : value.points ?? value.rows ?? value.data;
        db.exec('BEGIN IMMEDIATE');
        try {
      ...
      read(id, action, { maxAgeMs = action === 'quote' ? 45_000 : 615_000, fresh = false } = {}) {
