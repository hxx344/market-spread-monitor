...
开仓检查只估算公开盘口，不含下单步长、最小订单金额、价格保护、撤单变化或手续费；平仓收益由下述独立检查处理。现货汇率使用一档买卖价，未校验换汇深度；大额组合不可据此认定换汇一定能成交。返回的盘口总额仅代表已取档位，不能代表全市场容量。

## 当前盘口平仓测算

`POST /api/monitors/perpetual/exit` 接收两腿合约、完整身份、相同基础资产 `quantity`、`entryLongPrice`、`entryShortPrice`、`entryFeePaid`、`settledFunding`、可选投入本金 `capital`（未填为null）与平台 `takerOverrides`。每腿入场名义金额最多10,000,000 USDT。当前只支持两腿均以USDT计价及结算的线性合约；不推断跨币资金流或逆向合约损益。

退出多腿卖入买盘，退出空腿买入卖盘，按同一基础资产数量求各自逐档均价，与开仓方向相反。价格盈亏为 `quantity × (多腿退出均价 − 多腿开仓均价 + 空腿开仓均价 − 空腿退出均价)`；净收益再减已付开仓费用、减按本次退出实际成交金额和taker费率估算的两腿平仓费用，加用户登记的累计已结算资金费。资金费收入为正、支出为负，预估费率不会自动当成到账。

净收益同时显示USDT金额、以多腿开仓名义金额为分母的收益率，以及填写两账户投入本金后的本金收益率。盘口费用已随退出价格变化，不能直接复用四次固定名义费用；退出滑点已经体现在逐档均价内，不重复减一个滑点预算。任一腿深度不足或盘口过期时不显示完整总盈亏；费用未知时可保留价格盈亏，但净收益为空。结果最多有效10秒，重新点击不能刷新旧源时间。

退出检查与原开仓检查共享同一个合约盘口缓存、并发和限速预算，不新增常驻连接或盘口历史。检查只做估算，不执行交易；模拟或手工持仓登记及有界保存规则见[机会工作台](PERPETUAL_WORKSPACE.md)。
...
- 盘口、汇率和每次深度估算结果不持久化。用户另行登记的模拟/手工持仓使用独立有界记录；内存深度由上述市场数、档位数和队列数限定。
