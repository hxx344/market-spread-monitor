...
合约价差的完整双腿身份、连续时长和窗口占比不能用现有单指标梯度字段表达，因此使用 `app/perpetual-alerts.tsx` 的专用扩展区；继续共享统一飞书设置、登录同源校验与配置版本冲突。原油与海力士仍使用下述统一梯度编辑器，不在其他模块复制配置。

合约持续条件读取常驻服务的实时内存盘口（行情缓存每 15 秒刷盘），先检查源时间和写盘健康，在发送前持久化发送意图和去重状态，再次核验实时条件。不能改为让默认 10 秒新鲜度的持续检查只读取 15 秒前的磁盘快照。该例外只适用于合约持续条件；原有模块仍沿用各自落盘后检查流程。
