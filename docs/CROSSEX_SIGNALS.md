...
在合约价差的“价差排名”“全部报价”及展开的各平台报价中，直接点击币种旁的“屏蔽推送”即可停止该币种的新机会推送；所有同币种行同步显示“已屏蔽推送”，可点击“恢复推送”解除，行情行继续保留。页面上方“CrossEx 推送筛选”共用该名单，也支持输入基础币种（如 `BTC`）添加和逐个解除。所有入口共享一次设置轮询及保存锁，保存中禁用修改，服务器确认后更新状态；失败或配置冲突会显示原因并重新读取实际配置，待添加文本保留。
